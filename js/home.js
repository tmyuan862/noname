(function () {
  ...
  if (feedbackForm) {
    ...
    var refreshRepliesButton = feedbackForm.querySelector("[data-refresh-replies]");
    var receiptStorageKey = "mengyuan_feedback_receipts";

    function getReceipts() {
      try {
        return JSON.parse(localStorage.getItem(receiptStorageKey) || "[]");
      } catch (_) {
        return [];
      }
    }
    ...
    function saveReceipt(ticket, key) {
      var receipts = getReceipts().filter(function (item) {
    ...
    function loadReplies() {
      ...
      receipts.forEach(function (receipt) {
        fetch(
          "/api/feedback/reply?ticket=" +
            encodeURIComponent(receipt.ticket) +
            "&key=" +
            encodeURIComponent(receipt.key),
          { cache: "no-store" }
        )
          ...
          .then(function (data) {
            ...
            article.className = "feedback-receipt";

            var head = document.createElement("header");
            var category = document.createElement("strong");
            category.textContent = record.category;
            var state = document.createElement("span");
            state.textContent = record.reply ? "已回复" : "等待回复";
            head.append(category, state);

            var original = document.createElement("p");
            original.textContent = record.message;
            article.append(head, original);

            if (record.reply) {
              var answer = document.createElement("blockquote");
              answer.textContent = record.reply;
    ...
    if (refreshRepliesButton) {
      refreshRepliesButton.addEventListener("click", loadReplies);
    }
    loadReplies();
  }
  ...
  if (publicFeedbackList) {
    ...
    function loadPublicFeedback(append) {
      fetch("/api/feedback/public?page=" + publicFeedbackPage, { cache: "no-store" })
        .then(function (response) {
          if (!response.ok) throw new Error();
          return response.json();
        })
        .then(function (data) {
          ...
          if (!data.feedback.length && !append) {
            publicFeedbackList.innerHTML = "<p>还没有公开反馈。</p>";
          }

          data.feedback.forEach(function (record) {
            var article = document.createElement("article");
            article.className = "public-feedback-card";
            ...
            if (record.reply) {
              var reply = document.createElement("blockquote");
              reply.textContent = "站长回复：" + record.reply;
        ...
        .catch(function () {
          if (!append) {
            publicFeedbackList.innerHTML = "<p>反馈历史暂时无法读取。</p>";
          }
        });
    }

    publicFeedbackMore.addEventListener("click", function () {
      publicFeedbackPage += 1;
      loadPublicFeedback(true);
    });

    document.addEventListener("feedback:submitted", function () {
      publicFeedbackPage = 1;
  ...
  var revealTargets = document.querySelectorAll(
    ".hero-grid, .quick-section, .focus-section .section-shell, .advantage-section, .feedback-layout, .public-feedback-section"
  );
