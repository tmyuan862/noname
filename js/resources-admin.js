(function () {
  ...
  var complianceNote = document.querySelector("[data-compliance-note]");
  ...
  var analysisMode = "ai";
  var currentComplianceMode = "standard";
  ...
  function resetComplianceMode() {
    currentComplianceMode = "standard";
    if (complianceNote) complianceNote.hidden = true;
  }

  function fillPreview(draft, complianceMode) {
    currentComplianceMode = complianceMode || "standard";
    if (complianceNote) complianceNote.hidden = currentComplianceMode !== "summary_only";
  ...
  function load() {
    ...
    request("/api/admin/resources").then(function (data) {
      ...
      if (!data.resources.length) {
        list.innerHTML = '<p class="empty-state">还没有资料，可以粘贴通知或上传 JSON 文件。</p>';
        return;
      }
      ...
      list.innerHTML = '<p class="loading-state error-state">无法读取资料，请确认后台服务仍在运行。</p>';
    });
  }

  input.addEventListener("change", function () {
    var files = Array.from(input.files);
    importButton.disabled = !files.length;
  ...
  importButton.addEventListener("click", async function () {
    ...
    try {
      for (var file of files) {
        ...
        var items = JSON.parse(await file.text());
        if (!Array.isArray(items)) throw new Error(file.name + " 不是 JSON 数组");
        var result = await request("/api/admin/resources/import", {
          method: "POST",
      ...
      uploadStatus.textContent = "导入完成：新增 " + totals.inserted + "，更新 " + totals.updated + "，跳过 " + totals.skipped;
      input.value = "";
      load();
    } catch (error) {
      uploadStatus.className = "upload-status error";
      uploadStatus.textContent = error.message || "导入失败";
    } finally {
  ...
  analyzeButton.addEventListener("click", function () {
    ...
    if (text.length < 10) {
      setPasteStatus("请先粘贴一段完整通知。", "error");
      return;
    }
    resetComplianceMode();
    analyzeButton.disabled = true;
    setPasteStatus(analysisMode === "ai" ? "DeepSeek 正在整理内容…" : "正在本地识别标题、日期、部门和分类…");
    request("/api/admin/resources/analyze", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: text, mode: analysisMode })
    }).then(function (data) {
      fillPreview(data.draft, data.compliance_mode);
      setPasteStatus(
        data.analysis_mode === "local_fallback"
          ? data.message
          : (data.analysis_mode === "ai" ? "AI 增强分析完成，请核对后发布。" : "本地分析完成，请核对后发布。"),
        data.analysis_mode === "local_fallback" ? "error" : "success"
      );
    }).catch(function (error) {
      setPasteStatus(error.message || "分析失败，请稍后再试。", "error");
    }).finally(function () {
  ...
  analyzeUrlButton.addEventListener("click", function () {
    ...
    if (!/^https?:\/\//i.test(url)) {
      setPasteStatus("请输入完整的 http 或 https 网页链接。", "error");
      return;
    }
    ...
    request("/api/admin/resources/analyze-url", {
      ...
      if (data.compliance_mode === "summary_only") {
        setPasteStatus(data.message || "已切换为摘要发布模式，请核对后发布。", "success");
        return;
      }
      setPasteStatus(
  ...
  preview.addEventListener("submit", function (event) {
    ...
    var publishButton = preview.querySelector('[type="submit"]');
    publishButton.disabled = true;
    setPasteStatus("正在发布…");
    var draft = {};
    ["title", "category", "publish_date", "department", "url", "content"].forEach(function (name) {
      draft[name] = preview.elements.namedItem(name).value.trim();
    });
    ...
    var publishRequest = target === "senior"
      ? request("/api/admin/senior/import", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: draft.title, body: draft.content })
      })
      : request("/api/admin/resources/import", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    publishRequest.then(function (result) {
      if (target === "resources" && !result.inserted && !result.updated) {
        throw new Error("资料未通过校验，请检查标题、正文和官网链接。");
      }
      setPasteStatus(
        target === "senior"
          ? "已使用“梦缘校园整理员”发布到学长学姐说。"
          : (result.inserted ? "已发布一条新资料。" : "已更新相同信源的资料。"),
        "success"
      );
      resetComplianceMode();
      pasteText.value = "";
      ...
      setPasteStatus(error.message || "发布失败。", "error");
    }).finally(function () {
      publishButton.disabled = false;
    });
  });

  document.querySelector("[data-cancel-analysis]").addEventListener("click", function () {
    resetComplianceMode();
    preview.hidden = true;
    preview.reset();
    setPasteStatus("已取消本次分析。");
  });
  ...
  reformatButton.addEventListener("click", function () {
    ...
    request("/api/admin/resources/reformat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ limit: 100 })
    }).then(function (result) {
      setPasteStatus(
        "AI 排版整理完成：更新 " + result.updated + " 条" + (result.failed ? "，" + result.failed + " 条未修改。" : "。"),
        result.failed ? "error" : "success"
      );
      load();
    }).catch(function (error) {
      setPasteStatus(error.message || "AI 排版整理失败。", "error");
    }).finally(function () {
      reformatButton.disabled = false;
      reformatButton.textContent = "AI 整理全部排版";
    });
  });
